...
def test_inspect_valid(raw_image_rgb_png):
    ...
    assert sf.inspect(raw_image_rgb_png) == ("(0, 255, 255)(0, 255, 255)(0, 255, 255)" +
                                             "(0, 255, 255)\n" +
                                             "Filename: \nFormat: PNG\nMode: " +
                                             "RGB\nSize: 2x2\nValid: Yes")
...
def test_inspect_invalid_format(raw_image_rgb_jpeg):
    ...
    assert sf.inspect(raw_image_rgb_jpeg) == ("(0, 255, 255)(0, 255, 255)(0, 255, 255)" +
                                             "(0, 255, 255)\n" +
                                             "Filename: \nFormat: JPG\nMode: " +
                                             "RGB\nSize: 2x2\nValid: No")
...
def test_black_rgba_valid(raw_image_rgba_png):
    ...
    assert data == ["(0, 0, 0, 255)", "(0, 0, 0, 255)", "(0, 0, 0, 255)", "(0, 0, 0, 255)"]
...
def test_decode_valid_rgb_png_t(raw_coded_rgb_png, raw_image_rgb_png):
    '''Tests decoding on a valid RGB .png image, with lower case transparent mode.'''
    ...
    assert data == ["(255, 0, 0)", "(0, 255, 254)", "(0, 255, 254)", "(0, 255, 254)"]

def test_decode_valid_rgba_png_t(raw_coded_rgba_png, raw_image_rgba_png):
    '''Tests decoding on a valid RGBA .png image, with lower case transparent mode.'''
...
def test_decode_valid_rgb_png_ut(raw_coded_rgb_png, raw_image_rgb_png):
    '''Tests decoding on a valid RGB .png image, with upper case transparent mode.'''
    encoded = sf.encode(raw_coded_rgb_png, raw_image_rgb_png)
    encoded.format = "PNG"
    encoded.mode = "RGB"
    new_image = sf.decode(encoded, "T")
...
def test_decode_valid_rgba_png_ut(raw_coded_rgba_png, raw_image_rgba_png):
    '''Tests decoding on a valid RGBA .png image, with upper case transparent mode.'''
    encoded = sf.encode(raw_coded_rgba_png, raw_image_rgba_png)
    encoded.format = "PNG"
    encoded.mode = "RGBA"
    new_image = sf.decode(encoded, "T")
...
def test_decode_valid_rgb_png_b(raw_coded_rgb_png, raw_image_rgb_png):
    '''Tests decoding on a valid RGB .png image, with lower case black mode.'''
    encoded = sf.encode(raw_coded_rgb_png, raw_image_rgb_png)
    encoded.format = "PNG"
    encoded.mode = "RGB"
...
def test_decode_valid_rgba_png_b(raw_coded_rgba_png, raw_image_rgba_png):
    '''Tests decoding on a valid RGBA .png image, with lower case black mode.'''
    encoded = sf.encode(raw_coded_rgba_png, raw_image_rgba_png)
    encoded.format = "PNG"
    encoded.mode = "RGBA"
    new_image = sf.decode(encoded, "b")
...
def test_decode_valid_rgb_png_ub(raw_coded_rgb_png, raw_image_rgb_png):
    '''Tests decoding on a valid RGB .png image, with upper case black mode.'''
    encoded = sf.encode(raw_coded_rgb_png, raw_image_rgb_png)
    encoded.format = "PNG"
    encoded.mode = "RGB"
    new_image = sf.decode(encoded, "B")
...
def test_decode_valid_rgba_png_ub(raw_coded_rgba_png, raw_image_rgba_png):
    '''Tests decoding on a valid RGBA .png image, with upper case black mode.'''
    encoded = sf.encode(raw_coded_rgba_png, raw_image_rgba_png)
    encoded.format = "PNG"
    encoded.mode = "RGBA"
    new_image = sf.decode(encoded, "B")
...
def test_encode_valid_rgb_png(raw_coded_rgb_png, raw_image_rgb_png):
    ...
    assert data == ["(0, 255, 255)", "(0, 255, 254)", "(0, 255, 254)", "(0, 255, 254)"]
...
def test_encode_valid_rgb_coded_rgba_image_png(raw_coded_rgb_png, raw_image_rgba_png):
    '''Tests encoding both a valid RGB .png image inside a valid RGBA image'''
...
def test_encode_valid_rgba_coded_rgb_image_png(raw_coded_rgba_png, raw_image_rgb_png):
    '''Tests encoding both a valid RGBA .png image inside a valid RGB image'''
...
def test_encode_valid_rgb_smaller_png(raw_coded_smaller_rgb_png, raw_image_rgb_png):
...
def test_encode_invalid_rgb_coded_jpeg(raw_image_rgb_jpeg, raw_image_rgb_png):
    '''Tests encoding of an invalid RGB .jpeg image into a valid RGB .png image'''
    with pytest.raises(se.StegonosaurusIncorrectFormatException):
...
def test_encode_invalid_rgb_image_jpeg(raw_coded_rgb_png, raw_image_rgb_jpeg):
    '''Tests encoding of a valid RGB .png image into an invalid RGB .jpeg image'''
    with pytest.raises(se.StegonosaurusIncorrectFormatException):
        sf.encode(raw_coded_rgb_png, raw_image_rgb_jpeg)

def test_encode_invalid_l_coded_png(raw_image_l_png, raw_image_rgb_png):
    '''Tests encoding an invalid L .png image into an valid RGB .png image'''
    with pytest.raises(se.StegonosaurusIncorrectFormatException):
...
def test_encode_invalid_rgb_larger_png(raw_coded_larger_rgb_png, raw_image_rgb_png):
    '''Tests encoding an invalid larger RGB .png image into a valid RGB .png image'''
    with pytest.raises(se.StegonosaurusIncorrectSizeException):
        sf.encode(raw_coded_larger_rgb_png, raw_image_rgb_png)

def test_encode_invalid_rgb_larger_x_png(raw_coded_larger_x_rgb_png, raw_image_rgb_png):
    '''Tests encoding an invalid horizontally larger RGB .png image into a valid RGB .png image'''
    with pytest.raises(se.StegonosaurusIncorrectSizeException):
        sf.encode(raw_coded_larger_x_rgb_png, raw_image_rgb_png)

def test_encode_invalid_rgb_larger_y_png(raw_coded_larger_y_rgb_png, raw_image_rgb_png):
    '''Tests encoding an invalid vertically larger RGB .png image into a valid RGB .png image'''
    with pytest.raises(se.StegonosaurusIncorrectSizeException):
